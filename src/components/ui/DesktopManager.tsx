...
import { Link, FolderGit2, HardDrive, Folder, RefreshCw, Monitor, FileTerminal, Settings, Info, Briefcase, ChevronRight, ExternalLink, Globe, FileText, Search, Clock } from "lucide-react";
      ...
      {isMobile ? (
        ...
        <div className="absolute top-10 bottom-16 left-0 right-0 overflow-y-auto overflow-x-hidden">
          <div className="grid grid-cols-4 gap-3 p-4 pt-6">
            ...
            <DesktopIcon id="icon-resume" label="Resume" icon={<Briefcase size={24} />} onClick={() => window.open("https://drive.google.com/file/d/1-34NxUJF_Fj6-s4vUZVZIjIVO0VD-WX9/preview", "_blank")} />
          ...
          <div className="mt-8 px-4">
            <h3 className="text-xs font-bold text-foreground/50 uppercase tracking-wider mb-3 px-2">Featured Apps</h3>
            <div className="grid grid-cols-4 gap-3">
              <DesktopIcon id="icon-pgstudio" label="pgStudio" icon={<img src="https://github.com/dev-asterix/PgStudio/blob/main/docs/assets/postgres-explorer.png?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="pgStudio" />} onClick={() => openWindow("repo-demo", "pgStudio — Interactive Demo", 0, 0, { repoId: "pgStudio", maximized: true })} />
              <DesktopIcon id="icon-drawdown" label="drawdown" icon={<img src="https://github.com/dev-asterix/drawdown/blob/main/public/logo.png?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="drawdown" />} onClick={() => openWindow("repo-demo", "drawdown — Interactive Demo", 0, 0, { repoId: "drawdown", maximized: true })} />
              <DesktopIcon id="icon-andthetimeis" label="and-the-time-is" icon={<img src="https://github.com/dev-asterix/and-the-time-is/blob/main/public/favicon.ico?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="and-the-time-is" />} onClick={() => openWindow("repo-demo", "and-the-time-is — Interactive Demo", 0, 0, { repoId: "and-the-time-is", maximized: true })} />
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="absolute top-24 bottom-24 left-6 flex flex-col flex-wrap gap-6 items-start content-start z-0">
          ...
          <DesktopIcon
            ...
            onClick={() => window.open("https://drive.google.com/file/d/1-34NxUJF_Fj6-s4vUZVZIjIVO0VD-WX9/preview", "_blank")}
          />
          <DesktopIcon
            id="icon-notepad"
            label="Notepad"
        ...
        !isMobile && (
          <div className="absolute top-24 right-6 w-32 flex flex-col items-end gap-2 z-0 bg-background/20 backdrop-blur-md p-4 rounded-2xl border border-glass-border shadow-xl">
            <div className="w-full flex items-center justify-center mb-2 pb-2 border-b border-glass-border/50">
              <span className="text-[10px] uppercase tracking-widest font-bold text-foreground/60">Featured</span>
            </div>
            <DesktopIcon
              id="icon-pgstudio"
              label="pgStudio"
              icon={<img src="https://github.com/dev-asterix/PgStudio/blob/main/docs/assets/postgres-explorer.png?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="pgStudio" />}
              onClick={() => openWindow("repo-demo", "pgStudio — Interactive Demo", 220, 200, { repoId: "pgStudio", maximized: true })}
            />
            <DesktopIcon
              id="icon-drawdown"
              label="drawdown"
              icon={<img src="https://github.com/dev-asterix/drawdown/blob/main/public/logo.png?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="drawdown" />}
              onClick={() => openWindow("repo-demo", "drawdown — Interactive Demo", 240, 220, { repoId: "drawdown", maximized: true })}
            />
            <DesktopIcon
              id="icon-andthetimeis"
              label="and-the-time-is"
              icon={<img src="https://github.com/dev-asterix/and-the-time-is/blob/main/public/favicon.ico?raw=true" className="w-6 h-6 object-contain filter drop-shadow opacity-90" alt="and-the-time-is" />}
              onClick={() => openWindow("repo-demo", "and-the-time-is — Interactive Demo", 260, 240, { repoId: "and-the-time-is", maximized: true })}
            />
          </div>
        )
      }
        ...
        openWindows.map((win) => {
                            ...
                            win.type === "notepad" ? [660, 520] :
                              win.type === "imageviewer" ? [700, 560] :
                                win.type === "monitor" ? [720, 500] :
                                  win.type === "welcome" ? [1000, 850] :
                                    win.type === "repo-demo" ? [900, 650] :
                                      [400, 300]
          );
          ...
          return (
            ...
            >
              ...
              {win.type === "repo-demo" && win.metadata?.repoId && <RepoDemoViewer repoId={win.metadata.repoId} />}
